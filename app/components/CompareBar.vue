...
<template>
  <div
    ...
    class="fixed bottom-0 inset-x-0 z-40 bg-surface border-t border-white/10"
  >
    <div class="mx-auto max-w-6xl px-6 py-4 flex items-center justify-between gap-4 flex-wrap">
      <div class="flex items-center gap-3 flex-wrap">
        ...
        <span
          ...
          :key="item.slug"
          class="font-mono text-data-sm text-ink bg-canvas px-3 py-1.5 border border-white/10 flex items-center gap-2"
        >
          {{ item.label }}
      ...
      <div class="flex items-center gap-3 shrink-0">
        ...
        <NuxtLink
          ...
          class="bg-amber text-canvas font-mono text-eyebrow px-4 py-2 hover:bg-amber/90 transition-colors"
        >
          Uporedi
        </NuxtLink>
      </div>
